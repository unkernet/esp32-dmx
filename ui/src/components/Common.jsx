 ...
 */
export function Modal({ isOpen, title, children, onClose, onConfirm, confirmText = "Confirm", footer, class: className, busy }) {
  useEffect(() => {
    if (!isOpen) return;
        ...
        <footer>
          {footer ? footer : (
            <>
              ...
              {onConfirm && <button onClick={onConfirm} aria-busy={busy}>{confirmText}</button>}
            </>
          )}
        </footer>
      </article>
